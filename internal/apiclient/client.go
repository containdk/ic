...
type Cluster struct {
	...
	ClusterType *Type `json:"clusterType,omitempty"`
...
// ComponentCluster ComponentCluster models a cluster for inclusion
...
// ComponentResilienceZone ComponentResilienceZone models resilience zone for inclusion
...
type Problem struct {
	// Detail Detail is humanreadable explanation of the specific occurrence of the problem RFC-9457#3.1.4
...
// Type ClusterType specifies the type of cluster sharing
type Type = string
...
// Vulnerabilities Vulnerabilities represents the vulnerabilities detected in images running in a cluster
type Vulnerabilities struct {
	...
	// Count Count is the number of vulnerabilities in the collection
	Count *int32 `json:"count,omitempty"`

	// Vulnerabilities Vulnerabilities is the list of vulnerabilities ordered by descending severity
	Vulnerabilities *[]Vulnerability `json:"vulnerabilities,omitempty"`
}

// Vulnerability Vulnerability represents a single vulnerability found in one or more images running in a cluster
type Vulnerability struct {
	// Categories Categories is the sorted list of distinct workload categories affected, i.e., customer, platform or system
	Categories *[]string `json:"categories,omitempty"`

	// Description Description is the description of the vulnerability
	Description *string `json:"description,omitempty"`

	// Id ID is the vulnerability identifier, e.g., a CVE or GHSA identifier
	Id *string `json:"id,omitempty"`

	// Images Images is the list of images in the cluster containing the vulnerability
	Images *[]VulnerabilityImage `json:"images,omitempty"`

	// Links Links is a list of references to further information on the vulnerability
	Links *[]string `json:"links,omitempty"`

	// Package Package is the vulnerable package
	Package *VulnerabilityPackage `json:"package,omitempty"`

	// Severity Severity is the severity level of the vulnerability. One of: CRITICAL, HIGH, MEDIUM, LOW, NEGLIGIBLE, UNKNOWN
	Severity *string `json:"severity,omitempty"`

	// Workloads Workloads is the list of workloads in the cluster running images containing the vulnerability
	Workloads *[]VulnerabilityWorkload `json:"workloads,omitempty"`
}

// VulnerabilityImage defines model for VulnerabilityImage.
type VulnerabilityImage struct {
	// Digest Digest is the image digest
	Digest *string `json:"digest,omitempty"`

	// Ref Ref is the image reference
	Ref *string `json:"ref,omitempty"`
}

// VulnerabilityPackage Package is the vulnerable package
type VulnerabilityPackage struct {
	// FixVersions FixVersions is the list of versions fixing the vulnerability
	FixVersions *[]string `json:"fixVersions,omitempty"`

	// Name Name is the name of the package
	Name *string `json:"name,omitempty"`

	// Version Version is the installed version of the package
	Version *string `json:"version,omitempty"`
}

// VulnerabilityWorkload defines model for VulnerabilityWorkload.
type VulnerabilityWorkload struct {
	// ApiVersion APIVersion is the api version of the workload
	ApiVersion *string `json:"apiVersion,omitempty"`

	// Category Category is the category of the workload namespace, i.e., customer, platform or system
	Category *string `json:"category,omitempty"`

	// Kind Kind is the kind of the workload
	Kind *string `json:"kind,omitempty"`

	// Name Name is the name of the workload
	Name *string `json:"name,omitempty"`

	// Namespace Namespace is the namespace of the workload
	Namespace *string `json:"namespace,omitempty"`
}
...
type ClientInterface interface {
	...
	ListVulnerabilities(ctx context.Context, clusterId string, reqEditors ...RequestEditorFn) (*http.Response, error)
...
func (c *Client) ListVulnerabilities(ctx context.Context, clusterId string, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewListVulnerabilitiesRequest(c.Server, clusterId)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func NewListVulnerabilitiesRequest(server string, clusterId string) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/clusters/%s/vulnerabilities", pathParam0)
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
type ClientWithResponsesInterface interface {
	...
	ListVulnerabilitiesWithResponse(ctx context.Context, clusterId string, reqEditors ...RequestEditorFn) (*ListVulnerabilitiesResponse, error)
...
type ListVulnerabilitiesResponse struct {
	...
	ApplicationldJSON500          *Problem
	ApplicationproblemJSON500     *Problem
	ApplicationldJSONDefault      *Vulnerabilities
	ApplicationproblemJSONDefault *Vulnerabilities
}

// Status returns HTTPResponse.Status
func (r ListVulnerabilitiesResponse) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
func (c *ClientWithResponses) ListVulnerabilitiesWithResponse(ctx context.Context, clusterId string, reqEditors ...RequestEditorFn) (*ListVulnerabilitiesResponse, error) {
	rsp, err := c.ListVulnerabilities(ctx, clusterId, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseListVulnerabilitiesResponse(rsp)
}
